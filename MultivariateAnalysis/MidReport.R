sale=read.table(file.choose(),header=T)
rawdata.sale=sale
...
p <- NULL
for(i in 1:7){
   p <-c(p,ks.test(scale(sale[,i],center=T,scale=T),"pnorm")$p.value)
}
p
...
library(MASS)
sale=rawdata.sale
bc=boxcox(sale[,6]~c(1:50),lambda = seq(-5, 5, 1/100),data=sale)
#看圖發現最好的值在0~4之間
...
{
  temp=((sale[,6])^i-1)/i
  p6=c(p6,ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value)
  if(ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value>maxnum){
    bestlamda=i
    maxnum=ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value
  }
}
bestlamda
maxnum
...
glass=read.table(file.choose(),header=T)
rawdata.glass=glass

glass.test=read.table(file.choose())

write.csv(glass,"glass.dat.csv")
...
library(rpart)
glass=rawdata.glass
#?rpart.control有變數解釋  老師的有個地方打錯了 minsplit打成minisplit
...
plot(glass.treeorig, uniform=T, branch=0, margin=0.01, main="Classification Tree")
text(glass.treeorig,col="red")
printcp(glass.treeorig)

glass.prunetree<-prune.rpart(glass.treeorig,cp=0.02)
plot(glass.prunetree, uniform=T, branch=0, margin=0.01, main="Prune Classification Tree")
...
glass.prunetree<-prune.rpart(glass.treeorig,cp=0.02)
plot(glass.prunetree, uniform=T, branch=0, margin=0.01, main="Classification Tree")
text(glass.prunetree,col="red")
printcp(glass.prunetree)
...
#下面這裡也是 左邊是true Classification 上面是他分的

#cross validation
glass.ldacv<-lda(V10~V1+V2+V3+V4+V5+V6+V7+V8+V9,data=glass,CV=T)
...
p <- NULL
for(i in 1:9){
  p <-c(p,ks.test(scale(glass[,i],center=T,scale=T),"pnorm")$p.value)
}
p

#0.03986241       0.1058399      7.965134*10^-08     0.0706628         0.1302882 
#1.59163810^-11   0.001322527        0            2.220446*10^-16
...
#只有1跟7 其他不理我 都已經這麼大範圍還沒有好的lamda應該就是不管如何轉換都不會有常態了
glass=rawdata.glass
p7=NULL
maxnum2=0
bestlamda7=0
for(i in seq(-1.2, 0.6, 1/100))
{
  if (i==0)
    ...
    temp=((glass[,7])^i-1)/i
  p7=c(p7,ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value)
  if(ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value>maxnum2){
    bestlamda7=i
    maxnum2=ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value
  }
}
maxnum2
bestlamda7

#glass[,7]=((glass[,7])^bestlamda7-1)/bestlamda7
#轉換之後還是拒絕  就不轉了
...
for(i in seq(-85,-10, 1/100))
{
  if (i==0)
    ...
    temp=((glass[,1])^i-1)/i
  p1=c(p1,ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value)
  if(ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value>maxnum1){
    bestlamda1=i
    maxnum1=ks.test(scale(temp,center=T,scale=T),"pnorm")$p.value
  }
}
maxnum1
bestlamda1
...
#這邊我QDA是用有符合常態的 轉換的V1  原本的V2V4V5
#V10~.是對全部V1~V9 不是常態的要拿掉

glass.qda<-qda(V10~V1+V2+V4+V5,data=glass)

#glass.q<-read.table("newfish.qdat",h=T)
#上面那行應該不需要 
...
#cross validation
glass.qdacv<-qda(V10~V1+V2+V4+V5,data=glass,CV=T)
...
glass.knncv<-knn.cv(glass[,1:9],glass[,"V10"],k=3,prob=T)
...
glass.knn<-knn(glass[,1:9],glass[,1:9],glass[,"V10"],k=2,prob=T)
table(glass$V10,glass.knn)
(sum(table(glass$V10,glass.knncv))-table(glass$V10,glass.knn)[1,1]
                                  -table(glass$V10,glass.knn)[2,2]
                                  -table(glass$V10,glass.knn)[3,3]
                                  ...
                                  -table(glass$V10,glass.knn)[5,5]
                                  -table(glass$V10,glass.knn)[6,6])
glass.knncv<-knn.cv(glass[,1:9],glass[,"V10"],k=2,prob=T)
table(glass$V10,glass.knncv) 
(sum(table(glass$V10,glass.knncv))-table(glass$V10,glass.knncv)[1,1]
                                  -table(glass$V10,glass.knncv)[2,2]
                                  -table(glass$V10,glass.knncv)[3,3]
...
glass.knn<-knn(glass[,1:9],glass[,1:9],glass[,"V10"],k=1,prob=T)
table(glass$V10,glass.knn)
(sum(table(glass$V10,glass.knncv))-table(glass$V10,glass.knn)[1,1]
                                  -table(glass$V10,glass.knn)[2,2]
                                  -table(glass$V10,glass.knn)[3,3]
                                  ...
                                  -table(glass$V10,glass.knn)[5,5]
                                  -table(glass$V10,glass.knn)[6,6])
glass.knncv<-knn.cv(glass[,1:9],glass[,"V10"],k=1,prob=T)
table(glass$V10,glass.knncv) 
(sum(table(glass$V10,glass.knncv))-table(glass$V10,glass.knncv)[1,1]
                                  -table(glass$V10,glass.knncv)[2,2]
                                  -table(glass$V10,glass.knncv)[3,3]
...
glass.knntest<-knn(glass[,1:9],glass.test,glass[,"V10"],k=1,prob=T)
glass.knntest
...
glass.ldacv<-lda(V10~V1+V2+V4+V5,data=glass,CV=T)
table(glass$V10,glass.ldacv$class)

glass.knncv<-knn.cv(glass[,c(1,2,4,5)],glass[,"V10"],k=1,prob=T)
table(glass$V10,glass.knncv) 
...
glass.control=rpart.control(minsplit=0,minbucket=0,cp=0.01)
# the same tree was used before #
glass.adaboost=boosting(V10~., data=glass, boos=F, mfinal=100,control=glass.control)
glass.adaboost.pred=predict.boosting(glass.adaboost,glass)
glass.adaboost.pred$confusion

glass.adaboostcv=boosting.cv(V10~., data=glass,v=114,boos=F,mfinal=30,control=glass.control)
glass.adaboostcv[-1]

#上面boo=F全部都有錯誤 Error: object 'err' not found


glass.adaboost=boosting(V10~., data=glass, boos=T, mfinal=100,control=glass.control)
glass.adaboost.pred=predict.boosting(glass.adaboost,glass)
glass.adaboost.pred$confusion
#下面兩個結果一樣
glass.adaboostcv=boosting.cv(V10~., data=glass,v=114,boos=T,mfinal=30,control=glass.control)
glass.adaboostcv[-1]
